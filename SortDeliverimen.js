...
class SortDeliverymen {
  handleSort(merchant, deliverymen) {
    if (merchant.type === "lunchbox" && merchant.locale === "Brasilia") {
      const a = deliverymen.sort(sortByDesc("level"));

      return a;
    } else if (merchant.type === "lunchbox") {
      const a = deliverymen.sort(sortByAsc("avgDeliveryTime"));

      return a;
    } else if (merchant.type === "jewelry") {
      return deliverymen.sort(sortByDesc("level"));
    } else if (merchant.type === "drugstore") {
      return deliverymen.sort(sortByAsc("averageDeliveryCost"));
    }
  }
}
